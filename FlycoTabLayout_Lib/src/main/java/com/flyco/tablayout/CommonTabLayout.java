...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    /**
     * 更新数据 用于解决记录切换太快，刷新问题，当刷新时，将事件去除掉
     */
    public void notifyDataSetChanged(boolean isCancheck) {
        int childCount = mTabsContainer.getChildCount();
        for (int i = 0; i < childCount; i++) {
            mTabsContainer.getChildAt(i).setOnClickListener(isCancheck ? new OnClickListener() {
                @Override
