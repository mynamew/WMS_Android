 ...
 * 自动换行布局 （检验结果）
 * author: timi
 * create at: 2017/10/21 9:37
 */
public class WrapLayoutQualityResult extends ViewGroup {
    ...
    public WrapLayoutQualityResult(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public WrapLayoutQualityResult(Context context, AttributeSet attrs, int defStyle) {
     ...
     * @param mb       下外边距
     */
    public void setData(List<GetAdvance2Data.CheckItemBean> data, Context context, int textSize, int pl, int pt, int pr, int pb, int ml, int mt, int mr, int mb) {
        List<GetAdvance2Data.CheckItemBean> mydata = new ArrayList<>();
        if (data != null) {
        ...
        createChild(mydata, context, textSize, pl, pt, pr, pb, ml, mt, mr, mb);
    }
    ...
    private void createChild(List<GetAdvance2Data.CheckItemBean> data, final Context context, int textSize, int pl, int pt, int pr, int pb, int ml, int mt, int mr, int mb) {
        ...
        for (int i = 0; i < size; i++) {
             ...
             * 设置检验结果的标题
             */
            view.setTitle(data.get(i).getCheckItemName());
            view.setPadding(dip2px(context, pl), dip2px(context, pt), dip2px(context, pr), dip2px(context, pb));
